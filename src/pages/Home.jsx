...
const Home = () => {
  ...
  const closeForm = async (form) => {
    try {
      console.log("Attempting to close form:", form);
      
      // First, we need to query for the document with this form ID to get its document ID
      const formQuery = query(collection(db, "forms"), where("id", "==", form.id));
      const querySnapshot = await getDocs(formQuery);
      
      if (querySnapshot.empty) {
        console.error("No form found with id:", form.id);
        alert("Error: Could not find the form to close.");
        return;
      }
      
      // Get the actual Firestore document ID
      const docId = querySnapshot.docs[0].id;
      console.log("Found form document with ID:", docId);
      
      // Create a reference using the document ID
      const formRef = doc(db, "forms", docId);
      ...
      setForms(prevForms => prevForms.map(f => 
        f.id === form.id ? { ...f, closed: true, active: false, closedAt: new Date() } : f
      ));
      ...
      alert("Failed to close the form. Please try again.");
    }
  };
                  ...
                  >
                    <div className="p-5 bg-indigo-50 border-b border-gray-100">
                      <div className="flex justify-between items-center">
                        ...
                        {(form.closed || form.active === false) && (
                          <span className="bg-red-100 text-red-800 text-xs font-semibold px-2 py-1 rounded-full">
                            {form.closed ? "Closed" : "Disabled"}
                          </span>
                        )}
                      </div>
                    ...
                    <div className="p-5">
                      <div className="text-sm text-gray-600 mb-4 flex justify-between items-center">
                        ...
                        {form.closed && form.closedAt && (
                          <span className="text-xs text-gray-500">
                            Closed on {new Date(form.closedAt.seconds * 1000).toLocaleDateString()}
                          </span>
                        )}
                      </div>
                      ...
                      <div className="flex flex-col space-y-2">
                        ...
                        <button
                          ...
                          className={`w-full ${form.closed ? 'bg-gray-300 text-gray-600' : 'bg-gray-100 hover:bg-gray-200 text-gray-800'} font-medium py-2 px-4 rounded-md transition-colors duration-150 flex items-center justify-center`}
                          disabled={form.closed}
                        >
                          <svg xmlns="http://www.w3.org/2000/svg" 
                               className={`h-4 w-4 mr-2 ${form.closed ? 'text-gray-500' : 'text-red-500'}`} 
                               fill="none" 
                               viewBox="0 0 24 24" 
                               stroke="currentColor">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
                          </svg>
                          {form.closed ? "Form Closed" : "Close Form"}
                        </button>
                        <button
                          id={`copy-${form.id}`}
